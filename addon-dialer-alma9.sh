...
echo "Vicidial installation AlmaLinux/RockyLinux with CyburPhone and Dynamic portal - Add on dialers"
...
yum in -y initscripts perl-Curses.x86_64
...
mysql -u cron -h $DB  -p << MYSQLCREOF
...
sed -i s/DBSERVER/$DB/g /etc/astguiclient.conf
...
#0 1 * * * /usr/share/astguiclient/AST_CRON_audio_4_ftp2.pl --ftp-server=server.ip --ftp-login=user --ftp-pass=pass --ftp-directory=/ --ftp-persistent --ftp-validate --transfer-limit=100000 --list-limit=1
